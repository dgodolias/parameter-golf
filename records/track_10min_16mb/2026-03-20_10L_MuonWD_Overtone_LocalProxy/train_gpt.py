...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.num_kv_heads != self.num_heads and not _SDPA_SUPPORTS_GQA:
            ...
            if self.num_kv_heads != self.num_heads and _SDPA_SUPPORTS_GQA:
...
def eval_val_sliding(
    ...
    if max_val_seqs > 0:
        total = min(total, max_val_seqs * seq_len)
...
def main() -> None:
    ...
    args = Hyperparameters()
    record_dir = Path(__file__).parent.resolve()
    output_dir = Path(os.environ.get("OUTPUT_DIR", str(record_dir))).resolve()
    ...
    logfile = None
    if master_process:
        log_name = os.environ.get("LOG_FILE", f"{args.run_id}.txt")
        logfile = output_dir / log_name
        print(logfile)
    ...
    if eval_stride > 0:
        ...
        compiled_logits = torch.compile(base_model.forward_logits, dynamic=False) if use_compile else base_model.forward_logits
        # Warmup compilation
        ...
        q_val_loss, q_val_bpb = eval_val_sliding(
            ...
            eval_sl, eval_stride, max_val_seqs=args.max_val_seqs, eval_batch_seqs=eval_batch_seqs,
        )
        base_model.train()
    else:
